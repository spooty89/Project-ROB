...
enum customCharacterStateBackup3 {
	Idle = 0,
	Walking = 1,
	Trotting = 2,
	Running = 3,
...
function Update() {
	...
	if (Input.GetButton ("Vertical") && climbing)
	{
	...
	if (Input.GetButtonUp ("Vertical") && climbing)
	{
	...
	if (Input.GetButton ("Horizontal") && climbing)
	{
	...
	if (Input.GetButtonUp ("Horizontal") && climbing)
	{
	...
	if (IsGrounded())
	{
		...
		if (climbing && decending) {
			climbing = false;
			transform.position.x += (wallFacing.x);
			transform.position.z += (wallFacing.z);
		}
		if (IsJumping())
		{
			jumping = false;
...
function OnControllerColliderHit (hit : ControllerColliderHit )
{
	...
	if (Mathf.Abs(hit.normal.y) > 0.3) {
		if ((hit.gameObject.tag == "Climb") && (hit.normal.y < -0.1)) {
			//hangContact = true;
			//Debug.Log(wallRight);
			if (!hanging) {
				hanging = true;
...
/*function OnTriggerEnter(other:Collider)
{
    ...
    {    
        climbContact = true;
        wallFacing = other.GetComponentInChildren(Transform)
