...
enum customCharacterStateBackupTwo {
	Idle = 0,
	Walking = 1,
	Trotting = 2,
	Running = 3,
...
private var _characterState : customCharacterStateBackupTwo;
...
var wallSlidingGravityEffect = 0.2;
...
private var wallSlideFacing = Vector3.zero;
...
function UpdateSmoothedMovementDirection ()
{
	...
	if (grounded)
	{
		...
		_characterState = customCharacterStateBackupTwo.Idle;
		
		// Pick speed modifier
		if (Input.GetKey (KeyCode.LeftShift) || Input.GetKey (KeyCode.RightShift))
		{
			targetSpeed *= runSpeed;
			_characterState = customCharacterStateBackupTwo.Running;
		}
		else if (Time.time - trotAfterSeconds > walkTimeStart)
		{
			targetSpeed *= trotSpeed;
			_characterState = customCharacterStateBackupTwo.Trotting;
		}
		else
		{
			targetSpeed *= walkSpeed;
			_characterState = customCharacterStateBackupTwo.Walking;
		}
...
function ApplyGravity ()
{
	if (isControllable)	// don't move player at all if not controllable.
	{
		...
		if ((IsJumping() || IsDoubleJumping()) && !jumpingReachedApex && verticalSpeed <= 0.0)
		{
			...
			_characterState = customCharacterStateBackupTwo.Jumping_After_Apex;
		...
		if (IsGrounded ()) {
			...
			if (wallSliding) {
				//moveDirection = wallSlideFacing;
				verticalSpeed -= gravity * Time.deltaTime * wallSlidingGravityEffect;
			}
			else
...
function DidJump ()
{
	if(IsJumping()) {
		doublejumping = true;
		_characterState = customCharacterStateBackupTwo.Double_Jumping;
	}
	else {
		jumping = true;
		_characterState = customCharacterStateBackupTwo.Jumping;
	}
...
function Update() {
	...
	if(_animation) {
		...
		{
			case customCharacterStateBackupTwo.Jumping:
				_animation[jumpPoseAnimation.name].speed = jumpAnimationSpeed;
				_animation[jumpPoseAnimation.name].wrapMode = WrapMode.ClampForever;
				_animation.CrossFade(jumpPoseAnimation.name);
			break;
			case customCharacterStateBackupTwo.Double_Jumping:
			...
			break;
			case customCharacterStateBackupTwo.Jumping_After_Apex:
			...
			break;
			case customCharacterStateBackupTwo.Wall_Sliding:
	...
	{
		if (wallSliding)
			transform.rotation = Quaternion.LookRotation(wallSlideFacing);
		else
		...
		//if (hit.gameObject.tag == "Platform") {
			if (Mathf.Abs(hit.normal.y) > 0.1) {
				...
				if (HasJumpReachedApex()) {
					wallSliding = true;
					wallSlideFacing = hit.normal;
					//Debug.Log(wallSlideFacing);
	        		_characterState = customCharacterStateBackupTwo.Wall_Sliding;
	        		if (!hasWallJumped) {
	        			jumping = false;
	        			doublejumping = false;
	        		}
